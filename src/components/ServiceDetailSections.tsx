...
const ServiceBlock = ({ code, title, description, details, image, imageAlt, reversed = false, highlight, id }: ServiceBlockProps) => (
  <section id={id} className="py-20 px-6 md:px-12">
    <div className="max-w-7xl mx-auto">
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center ${reversed ? "lg:[direction:rtl]" : ""}`}>
        ...
        >
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-semibold text-foreground mb-6 leading-[1.1]">{title}</h2>
          {highlight && (
            <div className="bg-accent/10 border-l-4 border-accent px-5 py-4 mb-6 rounded-xl">
              <p className="text-foreground font-display font-medium text-sm leading-relaxed">{highlight}</p>
            </div>
          )}
          ...
          <ul className="space-y-3 mb-8">
            {details.map((detail, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 bg-accent mt-2.5 shrink-0 rounded-full" />
  ...
  return (
    <>
      ...
      <ServiceBlock
        ...
        imageAlt="Медицинское оборудование"
